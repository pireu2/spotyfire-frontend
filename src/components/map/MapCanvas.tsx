...
export default function MapCanvas({ parcels, alerts = [], activeLayer = "standard", onParcelSelect }: MapCanvasProps) {
  ...
  const sectorCoordinates: Record<string, [number, number]> = {
    ...
    "Sighetu Marmației": [47.9300, 23.8966],
    "Carei": [47.6830, 22.4674],
    ...
    "Marea Neagră (Offshore)": [44.1500, 29.1000],
    "Sarmizegetusa": [45.6231, 23.3113],
    "Glina": [44.3833, 26.2500],
  ...
  const getAlertColor = (type: string) => {
    switch (type) {
      ...
      case "ndvi": return "#16a34a"; // green-600
  ...
  return (
    ...
    >
      ...
      {alerts.map((alert) => {
        const coords = sectorCoordinates[alert.sector];
            ...
            <Popup>
              <div className="p-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className={`w-2 h-2 rounded-full`} style={{ backgroundColor: getAlertColor(alert.type) }}></span>
                  <span className="font-bold capitalize">{alert.type === 'ndvi' ? 'Alertă NDVI' : `Alertă ${alert.type}`}</span>
                </div>
                <p className="text-sm font-medium">{alert.message}</p>
                <div className="text-xs text-slate-500 mt-2 flex justify-between">
                  <span>{alert.sector}</span>
                  <span>{new Date(alert.timestamp).toLocaleDateString("ro-RO")}</span>
                </div>
              </div>
            </Popup>
          </CircleMarker>
