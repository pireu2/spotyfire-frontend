  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-12 relative z-10 space-y-12">
        ...
        {/* Hectare Slider */}
        <div className="max-w-xl mx-auto bg-slate-900/50 backdrop-blur border border-slate-800 rounded-2xl p-8 space-y-6">
            <div className="flex justify-between items-center">
                <label className="text-lg font-medium">Suprafață Teren</label>
                <div className="bg-slate-800 px-4 py-1 rounded-lg border border-slate-700">
                    <span className="text-2xl font-bold text-green-400">{hectares}</span>
                    <span className="text-slate-400 ml-1">ha</span>
                </div>
            </div>
            <Slider
                value={[hectares]}
                onValueChange={(vals) => setHectares(vals[0])}
                max={200}
                step={1}
                min={1}
                className="py-4"
            ...
            <p className="text-xs text-slate-500 text-center">
                Glisează pentru a ajusta suprafața. Prețurile se actualizează automat.
                {hectares > 20 && <span className="text-orange-400 block mt-1"><AlertCircle className="h-3 w-3 inline mr-1"/> Se aplică tarif suplimentar pentru {hectares - 20} ha.</span>}
            </p>
        </div>
